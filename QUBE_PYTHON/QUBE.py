...
class QUBE:
    def __init__(self, port, baudrate):
        self.master = serial.Serial(port=port, baudrate=baudrate, timeout=0.1, bytesize=serial.EIGHTBITS)
        self.rpm = 0
        self.voltage = 0
        self.current = 0
        self.motorAngle = 0
    ...
    def readByte(self):
        return int.from_bytes(self.master.read(),'big')
    ...
    def getLogData(self, motorSetpoint, pendulumSetpoint, e_speed, e_angle, setSpeed, setAngle):
        return [
            self.motorAngle,
            motorSetpoint,
            self.pendulumAngle,
    ...
    def getPlotData(self, motorSetpoint, pendulumSetpoint):
        return [
            self.motorAngle, # Plot 1
            motorSetpoint, # Plot 1
            self.pendulumAngle, # Plot 2
            pendulumSetpoint, # Plot 2
            self.rpm, # Plot 3
            self.current, # Plot 4
